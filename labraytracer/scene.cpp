...
void Scene::render(LayerRAM* lr) const {
    ...
    for (P.x = 0; P.x < size_t(imageSize_[0]); P.x++) {
        for (P.y = 0; P.y < size_t(imageSize_[1]); P.y++) {
			...
			double MinusfurtherLambda = 0;
			double MinusclosestLambda = maxLambda;
			double closestfurtherLambda = 0;
            RayIntersection temp, closest,closestfar,fartemp,farclosest,farfarthest;
			
			for (auto& R : Minus_) {
				if (R->closestIntersection(ray, MinusclosestLambda, fartemp)) {
					if (fartemp.getLambda() < MinusclosestLambda) {
						MinusclosestLambda = fartemp.getLambda();
						farclosest = fartemp;
						R->furtherIntersection(ray, 1000.0, fartemp);
						MinusfurtherLambda = fartemp.getLambda();
						farfarthest = fartemp;

					}
				}
			}
			//farfarthest =  rayintersection furtherst away on minus
			//MinusfurtherLambda = lambda at above intersection
			//farclosest =  intersection closest on minus
			// MinusclosestLambda =  lamba at above intersection
            for (auto& R : renderables_) {
                if (R->closestIntersection(ray, closestLambda, temp)) {
                    if (temp.getLambda() < closestLambda) {

						if (temp.getLambda() > MinusclosestLambda && temp.getLambda() < MinusfurtherLambda) {
							R->furtherIntersection(ray, 1000.0, temp);
							if (temp.getLambda() > MinusfurtherLambda) {
								closest = farfarthest;
								closestLambda = MinusfurtherLambda;
								is_a_minus = true;
							}
						}
						else {
            ...
            for (auto& light : lights_) {
				...
				if (is_a_minus) {
					...
					for (size_t r = 0; r < Minus_.size(); r++) {
						if (Minus_[r]->anyIntersection(shadowRay, length(L))) {
							inShadow = true;
							break;
						}
					}
...
void Scene::addMinus(std::shared_ptr<Renderable> renderable) {
	Minus_.push_back(renderable);
}
