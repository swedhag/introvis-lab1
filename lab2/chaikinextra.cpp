		...
		*/
			...
			std::vector<vec3>& Curve)
		{
			...
			for (size_t i(0); i<MinNumDesiredPoints - 1; i++)
			{
				...
				{
					 vec3& LeftPoint = Curve[j];
					 vec3& RightPoint = Curve[(j + 1) % Curve.size()];
					//if (!(floor(100 * (Curve[(j + 1) % Curve.size()][1] - Curve[j][1]) * (Curve[j][0] - Curve[(j - 1) % Curve.size()][0])) == floor(100 * (Curve[j][1] - Curve[(j + 1) % Curve.size()][1]) * (Curve[(j + 1) % Curve.size()][0] - Curve[j][0])))) {
					 //if (!(floor(100 * (Curve[(j) % Curve.size()][1] - Curve[(j- 1)][1]) * (Curve[(j+ 1) % Curve.size()][0] - Curve[(j)][0])) == floor(100 * (Curve[j][1] - Curve[(j - 1)][1]) * (Curve[(j + 1) % Curve.size()][0] - Curve[j][0])))) {
					// if (!(floor(100 * (Curve[j][1] - Curve[j - 1][1]) * (Curve[j+1][0] - Curve[j][0])) == floor(100 * (Curve[j+1][1] - Curve[j][1]) * (Curve[(j][0] - Curve[j-1][0])))) {
					 //if (abs(( (Curve[(j + 1) % Curve.size()][1] - Curve[j][1]) * (Curve[j][0] - Curve[j - 1][0])) -((Curve[j][1] - Curve[j - 1][1]) * (Curve[(j + 1) % Curve.size()][0] - Curve[j][0])))>float(0.0000001)) {
					 const float t = 0.25;
						newCurve.push_back((1 - t) * LeftPoint + t * RightPoint);
						newCurve.push_back(t*LeftPoint + (1 - t) * RightPoint);
				...
				//	}
				/*	else {
						vec3& MiddlePoint = float( 0.5) *LeftPoint + float(0.5)*RightPoint;
						//vec3& MiddlePoint =[Curve]
						newCurve.push_back(MiddlePoint);
						//newCurve.push_back(LeftPoint);
						//newCurve.push_back(RightPoint);

					}*/
				}
				std::vector<vec3> finalCurve;
				finalCurve.reserve(newCurve.size()*1000000*MinNumDesiredPoints);
				finalCurve.push_back(newCurve[0]);
				for (size_t j(1); j < newCurve.size(); j++) {
					/*std::cout << 'k' << ((newCurve[(j + 1) % newCurve.size()][1] - newCurve[j][1]) * (newCurve[j][0] - newCurve[j - 1][0])) - ((newCurve[j][1] - newCurve[j - 1][1]) * (newCurve[(j + 1) % newCurve.size()][0] - newCurve[j][0])) << '\n';
					std::cout << "finalCurve " << finalCurve.size() << '\n';
					std::cout << "abs-1 " << abs(-1) << '\n';
					std::cout << "abs -0.0015 " << abs(float(-0.0015))<< '\n';*/

					//if ((abs(((newCurve[(j + 1) % newCurve.size()][1] - newCurve[j][1]) * (newCurve[j][0] - newCurve[j - 1][0])) - ((newCurve[j][1] - newCurve[j - 1][1]) * (newCurve[(j + 1) % newCurve.size()][0] - newCurve[j][0]))) > float(0.001))) {
					if ((newCurve[j-1][0] * (newCurve[j][1] - newCurve[(j + 1) % newCurve.size()][1])) + (newCurve[j][0] * (newCurve[(j+1) % newCurve.size()][1] - newCurve[j - 1][1])) + (newCurve[(j + 1) % newCurve.size()][0] * (newCurve[j-1][1] - newCurve[j][1])) !=0) {
						finalCurve.push_back(newCurve[j]);
					}
				}
				Curve.clear();
				Curve = finalCurve;
